...
async def expose_single_file(
    ...
    Args:
        file: Archivo a analizar (imagen/video - selector de archivo en Swagger)
        prompt: Prompt personalizado para el análisis (opcional)
    ...
    Uso en Swagger:
        ...
        2. Click en "Choose File" para seleccionar imagen
    ...
    Response:
        {
            "status": "success",
            "filename": "imagen.jpg",
            "public_url": "https://abc123.ngrok.io/imagen.jpg",
            "analysis": "Análisis detallado de OpenAI...",
            "prompt_used": "Tu prompt personalizado",
            "model": "gpt-4o-mini",
    ...
    try:
        ...
        try:
            ...
            logger.info(f"   - Longitud respuesta: {len(analysis_result)} chars")
            ...
            logger.error(f"   ❌ Error en análisis OpenAI: {str(openai_error)}")
            # Retornar URL aunque falle el análisis
            return {
                ...
                "file_size_bytes": file_size,
                "note": "URL creada pero análisis falló. Verifica OPENAI_API_KEY"
            }

    except HTTPException:
        raise
    except Exception as e:
        logger.error(f"   ❌ Error exponiendo archivo: {str(e)}")
        raise HTTPException(
            status_code=500,
            detail=f"Error exposing file: {str(e)}"
        )
...
async def analyze_from_run_id(run_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        api_key = os.getenv("OPENAI_API_KEY") or os.getenv("OPEN_API_KEY")
        if not api_key:
            logger.error(f"   ❌ RUN_ID {run_id}: OPENAI_API_KEY no configurada")
            raise HTTPException(
                status_code=503,
...
async def analyze_local_only(
    ...
    try:
        ...
        # PASO 4: Exponer archivos con ngrok (más eficiente que base64)
        logger.info(
            "\n🌐 PASO 4/5: Exponiendo multimedia con ngrok...")
        ...
        # Procesar videos: extraer frames
        ...
        logger.info(f"   📹 Detectados {len(all_videos)} videos")

        total_frames_extracted = 0
        for video_path in all_videos[:5]:  # Procesar máx 5 videos
            try:
                ...
                logger.info(f"      ✅ {len(frames)} frames guardados")
            except Exception as e:
