...
try:
    ...
    print("Apify routes loaded successfully")
except Exception as e:
    print(f"WARNING: Apify routes not available: {e}")

# BigQuery and Drive routers intentionally removed (not used anymore)
...
gcloud_router = None
try:
    from deprecated.gcloud_routes import router as gcloud_router
    print("Google Cloud routes loaded successfully")
except Exception as e:
    print(f"WARNING: Google Cloud routes not available: {e}")

app = FastAPI(
    title="Ads Analyzer API Service",
    description="API para análisis de anuncios con Apify y Google Cloud Storage",
    version="1.0.0"
)

# Configuración de CORS para permitir requests desde Django
app.add_middleware(
    CORSMiddleware,
    allow_origins=["http://localhost:8000",
                   "http://127.0.0.1:8000"],  # URL de Django
...
# BigQuery and Drive routers intentionally not included
...
if gcloud_router:
    app.include_router(
        gcloud_router, prefix="/api/v1/gcloud", tags=["Google Cloud"])
